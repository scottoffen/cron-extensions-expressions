...
Every method replaces one field with an inclusive `start-end` range:
...
The day-of-week method is named `RangeOfWeek`. Its list counterpart on [`ListExtensions`](./list-extensions.md) is `OnDaysOfWeek`.
...
A few common contiguous-range schedules:
...
Validation happens immediately: the `RangeOf*` method assigns to the underlying property, and the property setter validates on assignment, so an invalid range throws from the `RangeOf*` call itself rather than later from `ToCronExpression()`. Ranges are inclusive of both bounds.
