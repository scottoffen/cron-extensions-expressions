...
| Range start not less than end (`6-3`, `5-5`)                       | `ArgumentOutOfRangeException`            | For `5-5`, assign the single value directly instead. For `6-3`, you likely want a range that crosses the field's wraparound point (e.g. Saturday through Wednesday) - ranges can't wrap in any cron dialect, so split it into two ascending ranges joined by a comma, or list the individual values. See [Ranges Don't Wrap](./cron-format.md#ranges-dont-wrap). |
